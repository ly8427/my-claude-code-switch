...
1. **避免 Auth 冲突**：新版 Claude Code（v2.1.159+）检测到 `ANTHROPIC_AUTH_TOKEN` 和 `ANTHROPIC_API_KEY` 同时设置会报 "Auth conflict"——即使值相同。因此 ccs 默认**只**输出 `ANTHROPIC_AUTH_TOKEN`（Bearer）。（极少数用旧版 CLI 的场景可设 `auth_mode: api_key`。）
2. **relay 端点 Bearer 提示**：非官方 base_url 时提醒 token 需走 `Authorization: Bearer`。
3. **SDK 模式模型提示**：`ANTHROPIC_MODEL` 在 Claude Agent SDK 下不会被自动读取，提示需显式传给 `ClaudeAgentOptions`；交互式 CLI 正常。
